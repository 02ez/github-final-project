...
interactive_mode() {
    ...
    while true; do
        read -rp "Enter principal amount: " P
    ...
    while true; do
        read -rp "Enter annual interest rate (%): " R
    ...
    while true; do
        read -rp "Enter time period (years): " T
        if validate_positive_number "$T" "time"; then
            break
        fi
    done
    
    read -rp "Use compound interest? (y/N): " compound_choice
    if [[ "$compound_choice" =~ ^[Yy]$ ]]; then
        COMPOUND_MODE=true
    fi
    
    read -rp "Currency code (default: USD): " currency_input
